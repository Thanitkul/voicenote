...
router.get('/get-recordings/:id', RouteProtection.verify, async (req, res, next) => {
    try {
        ...
        if(!isStudent) {
            ...
            res.status(200).json((await con.query("SELECT id, MIN(date(recordedAt)) recordedAtDate, MIN(time(recordedAt)) recordedAtTime, groupId FROM recordings WHERE courseId = ? GROUP BY groupId ORDER BY id", [req.params.id])))
        }
    } catch (err) {
        next(err)
    }
 ...
 */
router.delete('/leave-course', RouteProtection.verify, async(req, res) => {

  try {
    
      await con.query("DELETE FROM student_course WHERE studentId = ? AND courseId = ?", [[req.user.userId], req.body.courseId]);
